...
SOURCE_DIR = RAW_DIR if RAW_DIR.exists() else SEED_DIR
...
def main() -> None:
    if not SOURCE_DIR.exists():
        raise FileNotFoundError(
            f"Neither {RAW_DIR} nor {SEED_DIR} exists. "
            "Create data/seed (recommended for CI) or place raw files in data/raw."
        )
    ...
    prior_s = (
        prior_f.sample(n=min(N_PRIOR, len(prior_f)), random_state=SEED)
        if len(prior_f)
        else prior_f
    )
    ...
    train_s = (
        train_f.sample(n=min(N_TRAIN, len(train_f)), random_state=SEED)
        if len(train_f)
        else train_f
    )
